...
export const JMUApp = ({ gpa, history }: JMUAppProps) => {
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 animate-in slide-in-from-right duration-500 overflow-hidden">
      <div className="bg-gradient-to-r from-cyan-600 to-blue-700 px-8 py-8 text-white shrink-0">
        ...
        <h3 className="text-2xl font-black tracking-tight mb-2">江城传媒学院</h3>
      ...
      <div className="flex-1 p-6 overflow-y-auto custom-scrollbar pb-32 bg-slate-50/50">
        <div className="bg-white rounded-[2rem] p-8 shadow-sm border border-cyan-100/70 mb-6">
          <div className="flex justify-between items-center mb-8">
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Current Cumulative GPA</span>
            <div className="w-10 h-10 bg-cyan-50 rounded-xl flex items-center justify-center text-cyan-600"><Trophy size={20} /></div>
          </div>
        ...
        <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-cyan-100/70 mb-6 font-bold overflow-hidden">
          ...
          <div className="h-40 w-full pr-4">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={gpaTrend}>
                ...
                />
                <Area type="monotone" dataKey="val" stroke="#0891b2" strokeWidth={3} fillOpacity={1} fill="url(#colorGpa)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          {history.slice(-5).reverse().map((h, i) => (
            <div key={i} className="bg-white p-5 rounded-[1.4rem] border border-cyan-100/70 flex items-center gap-4 transition-all hover:scale-[1.02]">
